...
CMD ["sh", "-c", "cifuzz run test:test1 --use-sandbox=false > /PasswordStrength/fuzzing.log 2>&1 && cat /PasswordStrength/fuzzing.log && cifuzz finding && cifuzz coverage test1"]